...
Target concept: `docs/concept/mapping-theory.md` (spec) + `docs/concept/mapping-explained.md`
(rationale) + `docs/concept/skeleton-example.json` (golden example). Goal: AutoSys JIL and
Stonebranch UC exports that describe the same scheduling logic normalize to **byte-identical
canonical skeleton files**, so comparison becomes keyed line diff instead of graph matching.
...
- **Doc path bug.** Docs reference `data/skeleton-example.json`; the file lives at
  `docs/concept/skeleton-example.json` (fixed by task 09b).
